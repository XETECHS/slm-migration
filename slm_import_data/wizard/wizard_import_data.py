...
class WizardDataImport(models.TransientModel):
	...
	#is_delete = fields.Boolean('Delete data table')
	...
	def action_search_rows(self):
		...
		if self.type_transactions == 'edgar_app':
			...
			sql = ('''select count(sm.id) as number from slm_edgard sm where sm.sheet_name != 'SALES JOURN';''')
			cr.execute(sql,[])
			for row in cr.dictfetchall():
				num_rows += row['number']
			#if num_rows > 0:
	...
	def action_update_rows(self):
		...
		if self.type_transactions == 'edgar_app':
			...
			#raise UserError(_('%s')%(log_file))

		if 'warning' in log_file:
			#to close the current active wizard
			action = self.env.ref('slm_import_data.action_data_import_edgar').read()[0]
