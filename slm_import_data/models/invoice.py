...
class AccountInvoice(models.Model):
			...
			for row in cr.dictfetchall():
				...
				if move_lines:
					...
					try:
						...
						log_file += "\n"
						log_file += "Debe: %s Haber: %s Debe USD: %s Haber USD: %s" %(move_lines[6],move_lines[8],move_lines[7],move_lines[9])
						log_file += "\n"
						...
						log_file += move_lines[5]
						log_file += "\n"
						log_file += "Debe: %s Haber: %s Debe USD: %s Haber USD: %s" %(move_lines[6],move_lines[7],move_lines[8],move_lines[9])
						log_file += "\n"
						cr.rollback()
						pass
		except Exception as e:
	...
	def create_move_margo(self):
		...
		for row in cr.dictfetchall():
			move_lines = self.get_move_lines(row)
			#raise UserError(('%s') %(move_lines[0]))
		...
		try:
			if row:
				...
				for l in cr.dictfetchall():
					...
					day_book = l['day_book']
					date = l['date']
					amounts = self.get_amount(l.get('currency_code', ''), l.get('amount', 0.00), l.get('amount_srd', 0.00), l.get('amount_usd', 0.00),  datetime.strptime(str(date),'%Y-%m-%d %H:%M:%S').date())
					try:
					...
					if not account_id:
						...
						log_line += 'Row no {} - Account {} not found'.format(num_line, l['account_number'])
						log_line += "\n"
						pass
					if l['amount'] >= 0.00:
						debit += l['amount']
		...
		if currency_code and amount:
			if amount and amount_usd:
				...
				if currency_code == usd_code and not amount_usd:
					...
					currency_id = self.get_currency(currency_code)
					company_id = self.env['res.company'].search([('id', '=', 2)], limit=1)
					amount_usd = currency_id._convert(amount, company_id.currency_id, company_id, str(date_import) or fields.Date.today(), False)
	...
	def get_currency(self, currency_code):
		...
		currency_id = False
		if currency_code:
			currency_id = currency_obj.search([('currency_code', '=', currency_code)], limit=1)
		return currency_id
	...
	def get_account(self, account_number, branch_code):
		...
		if account_number:
			...
			account_id = account_obj.search([('code', '=', account_number), ('company_id', '=', 2)], limit=1)
	...
	def get_flight_number(self, flight_number_name):
		...
		if flight_number_name:
			flight_number = flight_number_obj.search([('name', '=', flight_number_name)], limit=1)
			if flight_number:
