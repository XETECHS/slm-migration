...
class PosBox(CashBox):
    ...
    def run(self):
        ...
        if active_model == 'cash.session':
            bank_statements = [session.cash_register_id for session in self.env[active_model].browse(
                active_ids) if session.cash_register_id]
...
#         active_model = self.env.context.get('active_model', False)
#         active_ids = self.env.context.get('active_ids', [])
#         if active_model == 'cash.session' and active_ids:
...
class PosBoxOut(PosBox):
    ...
    def _calculate_values_for_statement_line(self, record):
        values = super(
            PosBoxOut, self)._calculate_values_for_statement_line(record)
        active_model = self.env.context.get('active_model', False)
        active_ids = self.env.context.get('active_ids', [])
        if active_model == 'cash.session' and active_ids:
            values['ref'] = self.env[active_model].browse(active_ids)[0].name
