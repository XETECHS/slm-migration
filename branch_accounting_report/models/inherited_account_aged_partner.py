        ...
        for i in range(5):
            ...
            for line in self.env['account.move.line'].browse(aml_ids):
                ...
                for partial_line in line.matched_credit_ids:
                    if partial_line.max_date <= date_from:
                        partial_line_currency = partial_line.company_id.currency_id
                        line_amount -= partial_line_currency._convert(partial_line.amount, used_currency, user_company,
                                                                      date)

                if not self.env.user.company_id.currency_id.is_zero(line_amount) and partner_id:
