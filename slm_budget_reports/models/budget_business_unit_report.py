...
class BudgetBusinessUnitReport(models.AbstractModel):
    _name = "budget.business.unit.report"
    ...
    def _get_report_name(self):
        return _("Budget Business Unit Report")

    def _get_templates(self):
        templates = super(BudgetBusinessUnitReport, self)._get_templates()
    ...
    def _build_options(self, previous_options=None):
        if not previous_options:
            previous_options = {}
        options = super(BudgetBusinessUnitReport, self)._build_options(previous_options)
    ...
    def _set_context(self, options):
        ctx = super(BudgetBusinessUnitReport, self)._set_context(options)
        business_units = []
        if options.get('business_unit'):
