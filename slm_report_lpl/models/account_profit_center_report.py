...
class AccountProfitCenterReport(models.AbstractModel):
    _name = "account.profit.center.report"
    _description = "Profit Center Report"
    ...
    def _get_report_name(self):
        return _("Profit Center Report")

    def _get_templates(self):
        templates = super(AccountProfitCenterReport, self)._get_templates()
    ...
    def _set_context(self, options):
        ctx = super(AccountProfitCenterReport, self)._set_context(options)
        profit_centers = []
    ...
    def _get_options(self, previous_options=None):
        if not previous_options:
            previous_options = {}
        options = super(AccountProfitCenterReport, self)._get_options(previous_options)
        if options.get('profit_center'):
    ...
    def _do_query(self, options, line_id, limit=False):
        ...
        params = ((context.get('date_from'), context.get('date_to'), '(4|8|9)%') + params_profit_center +
                  (context.get('date_from'), context.get('date_to'))) * 4 + params_profit_center
    ...
    def _get_lines(self, options, line_id=None):
        ...
        # grouped_accounts_budget = self._resolve_formula_budget(options, line_id)
        context = dict(self._context or {})
        ...
        lines.append({
            ...
            'columns': ([w for v in profit_centers for w in
                         [{'name': v.name.upper(), 'style': 'text-align:right'}]] + [
                            {'name': ''}] if len(profit_centers) > 1 else []),
            'level': 1,
            'unfoldable': False,
        ...
        #     'title_hover': 'Profit Center',
        #     'columns': ([w for v in profit_centers for w in
        #                  [{'name': v.name.upper(), 'style': 'text-align:right'}, {'name': ''}, {'name': ''}]] + [
        #                     {'name': ''}, {'name': ''}, {'name': ''}] if len(profit_centers) > 1 else [{}, {}]),
        #     'level': 1,
        #     'unfoldable': False,
        #     'colspan': 2 if len(profit_centers) == 1 else 1,
        #     'style': 'text-align:left;font-size:14px;background-color:lightblue'
        ...
        lines.append({
            'id': 'header_profit_centers',
            'name': (parse(context.get('date_from')).strftime("(%B %Y T/M ") +
                     parse(context.get('date_to')).strftime("%B %Y) ")).upper(),
            'title_hover': '',
            'columns': [w for v in profit_centers for w in
                        [{'name': v.name + ' TOTAL', 'style': 'text-align:right'}]] + (
                           [{'name': 'TOTAL', 'style': 'text-align:right'}] if len(profit_centers) > 1 else []),
            'level': 1,
            'unfoldable': False,
            'colspan': 1,
            'style': 'font-size:14px;background-color:lightblue;text-align:center'
        })
        # lines.append({
        #     'id': 'header_profit_centers',
        #     'name': (parse(context.get('date_from')).strftime("(%B %Y T/M ") +
        ...
        #     'columns': [w for v in profit_centers for w in
        #                 [{'name': v.name + ' TOTAL', 'style': 'text-align:right'}, {}, {}]] + (
        #                    [{'name': 'TOTAL'}, {}, {}] if len(profit_centers) > 1 else []),
        #     'level': 1,
        #     'unfoldable': False,
        #     'colspan': 1,
        #     'style': 'font-size:14px;background-color:lightblue;text-align:center'
        # })
        # lines.append({
        #     'id': 'header_balance_budget',
        #     'name': '',
        #     'title_hover': '',
        #     'columns': [x for v in range(0, len(profit_centers) + (1 if len(profit_centers) > 1 else 0)) for x in
        #                 [{'name': 'BALANCE'}, {'name': 'BUDGET'}, {'name': 'DIFF'}]],
        #     'level': 2,
        #     'unfoldable': False,
        #     'colspan': 1,
    ...
    def _do_query_budget(self, options, line_id, limit=False):
        context = dict(self._context or {})
    ...
    def _do_query_overhead_budget(self, profit_center_id):
        sql = """
             WITH REPORT AS (
                SELECT * FROM profit_center_report PCR WHERE PCR.profit_center_mapping_id = 1 AND PCR.id = %s),
             OVERHEAD_BALANCE AS (
                 SELECT %s AS report_id,
                        SUM(planned_amount * (BEML.encryption / 100)) AS balance
        ...
        """
        context = dict(self._context or {})
        params = profit_center_id, profit_center_id, '6%', '%OVERHEAD%', context.get('date_from'), \
                 context.get('date_to')
        self.env.cr.execute(sql, params)
        results = self.env.cr.dictfetchall()
        return results

    def _resolve_formula_budget(self, options, line_id=None):
        context = dict(self._context or {})
        results = self._do_query_budget(options, line_id)
        for ipc, profit_center in enumerate(context['profit_centers']):
            results_by_code = {'E{}'.format(result['id']): result['balance'][ipc] for result in results}
            results_by_code['result'] = 0
            overhead = self._get_overhead_budget(profit_center)
            for i, result in enumerate(results):
                if result['type'] == 'overhead':
                    results[i]['balance'][ipc] = overhead['balance']
                    results[i]['name'] = '{} ({}%)'.format(results[i]['name'], overhead['overhead'])
    ...
    def _get_overhead_budget(self, profit_center_id):
        results = self._do_query_overhead_budget(profit_center_id)
        return results[0]
