...
class SlmFinancialReportsDefine(models.Model):
    ...
    report_line = fields.One2many('slm.financial.reports.define.lines', 'report_id', string='Report Lines', copy=True, auto_join=True)
    ...
    def get_renglon(self,report_name,renglon_code):
        ...
        result = 0
        define_id = self.env['slm.financial.reports.define'].search([('name','=',report_name)],limit=1)        
        if define_id:
            report_res = define_id._compute_report_balance(define_id.report_line)
            _logger.info('*****************///////////////////// >>>>>>>>>report_res: %s', report_res)
            for res in report_res:
                if date_to>='2019-07-31':                
                    _logger.info('res: %s', (report_res[res]['code'],report_res[res]['name'],report_res[res]['balance']))
...
class SlmDefineFinancialReportsLinesDefine(models.Model):
    ...
    report_id = fields.Many2one('slm.financial.reports.define', string='Report Reference', required=True, ondelete='cascade', index=True, copy=False, readonly=True)
    ...
    subgroup_ids = fields.Many2many('account.subgroup', string='Account Subgroups')
