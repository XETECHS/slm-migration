...
class ReportLayoutProfitLoss(models.AbstractModel):
    _name = 'report.slm_report_lpl.report_balancesheet'
    ...
    def _compute_account_balance(self, accounts2, filtro_origen):
        ...
        company_current = self.env['res.company'].search([('id','=',company_id)],limit=1)
        ...
        for cur in companies:
            ...
            if company_currency==company_current.currency_id:
                ...
                rate = company_currency._convert(1, currency, cur, date_to, round=False)
            ...
            if accounts:
                ...
                for row in self.env.cr.dictfetchall():
                    if res[row['id']]:
                        currency = company_current.currency_id.with_context(date=date_to)
                        company_currency = cur.currency_id
    ...
    def _compute_report_balance(self, reports):
        ...
        for report in reports:
            ...
            if report.formula:
                ...
                res[report.id]['subgroup'] = self._compute_subgroup_balance(res[report.id]['account'])                                      
            else:                            
    ...
    def get_account_lines(self, data):
        ...
        if tag_id:
            for report in child_reports:
                ...
                incomes = sum([subgroup_bal[x]['balance'] for x in subgroup_bal])
                incomes_budget = 0.00
                incomes_year_prior_bal = sum([year_prior_subgroup[x]['balance'] for x in year_prior_subgroup])
                incomes_month_prior_bal = sum([month_prior_subgroup[x]['balance'] for x in month_prior_subgroup])                  
            ...
            incomes = sum([res[x.id]['balance'] for x in child_reports])            
            incomes_budget = 0.00 #sum([res[x.id]['budget'] for x in child_reports])
            incomes_year_prior_bal = sum([res[x.id]['year_prior_bal'] for x in child_reports])
            incomes_month_prior_bal = sum([res[x.id]['month_prior_bal'] for x in child_reports])                            
            for report in child_reports:
                rep_sing = 1
