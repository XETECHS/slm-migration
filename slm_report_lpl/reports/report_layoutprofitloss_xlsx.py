...
class ReportLayoutProfitLossXls(models.AbstractModel):
    ...
    def get_account_lines(self, data):
        ...
        _logger.info('year_prior_context:%s'%(data.get('year_prior_context')))
        _logger.info('month_prior_context:%s'%(data.get('month_prior_context')))                  
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        if docs.enable_consolidate:
            selected_companies = self.env['res.company'].search(['!',('parent_id','=',docs.company_id.id),('id','=',docs.company_id.id)])
            companies = []
        ...
        if data['form'].get('tag_id', False):
            ...
            sheet.write(row_number,col_number + 1, '=SUM(C'+str(row_initial+2) +':C'+ str(row_sum-1) + ')-'+str(balance_dif), money)
            sheet.write(row_number,col_number + 4, '=SUM(F'+str(row_initial+2) +':F'+ str(row_sum-1) + ')', money)
            sheet.write(row_number,col_number + 7, '=SUM(I'+str(row_initial+2) +':I'+ str(row_sum-1) + ')-'+str(year_prior_bal_dif), money)
            sheet.write(row_number,col_number + 10, '=SUM(L'+str(row_initial+2) +':L'+ str(row_sum-1) + ')-'+str(month_prior_bal_dif), money)
