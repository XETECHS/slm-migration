...
class ReportLayoutProfitLossXls(models.AbstractModel):
    ...
    _name = 'report.slm_report_lpl.report_balancesheet.xlsx'
    ...
    def _compute_account_balance(self, accounts2, filtro_origen):
        ...
        base_current = self.env.user.company_id #self.env['res.company'].search([('id','=',1)],limit=1)
        company_current = self.env['res.company']._company_default_get('account')
        multi_company_ids=context.get('multi_company_ids')
        filtro = filtro_origen          
        res = {}
        if multi_company_ids:
            companies = self.env['res.company'].search([('id','in',multi_company_ids)])
        else:
            companies = self.env['res.company'].search([('id','=',company_id)])
        for cur in companies:
            ...
            if company_currency==company_current.currency_id:
                ...
                base_cur = 1
                rate_cur = base_current.currency_id._convert(1, cur.currency_id, cur, date_to, round=False)
                rate_cia = base_current.currency_id._convert(1, company_current.currency_id, cur, date_to, round=False)
                rate = company_currency._convert(base_cur, currency, base_current, date_to, round=False)
                #rate = rate_cia/rate_cur #company_currency._convert(base_cur, base_current.currency_id, base_current, date_to, round=False)
            rate_cur = base_current.currency_id._convert(1, cur.currency_id, cur, date_to, round=False)
            rate_cia = base_current.currency_id._convert(1, company_current.currency_id, cur, date_to, round=False)
            rate_act = company_current.currency_id._convert(1, cur.currency_id, cur, date_to, round=False)
            """
            ...
            if accounts:
                tables, where_clause, where_params = self.env['account.move.line'].with_context(company_id=cur.id)._query_get()
                ...
                for row in self.env.cr.dictfetchall():
                    if res[row['id']]:
                        ...
                        date_rate = (datetime.date(year + month//12, month % 12 + 1, 1) - datetime.timedelta(days=1))                        
                        if company_currency==company_current.currency_id:
                            rate = 1
                        else:
                            base_cur = 1
                            rate_cur = base_current.currency_id._convert(1, cur.currency_id, cur, date_rate, round=False)
                            rate_cia = base_current.currency_id._convert(1, company_current.currency_id, cur, date_rate, round=False)
                            rate = company_currency._convert(base_cur, currency, base_current, date_rate, round=False)                          
                        res[row['id']]['credit'] = res[row['id']]['credit'] + row['credit']*rate
                        res[row['id']]['debit'] = res[row['id']]['debit'] + row['debit']*rate
                        res[row['id']]['balance'] = res[row['id']]['balance'] + row['balance']*rate
                    else:
                        res[row.pop('id')].append(row)
                    
                tables, where_clause, where_params = self.env['account.move.line'].with_context(company_id=cur.id,initial_bal=True)._query_get()
                tables = tables.replace('"', '') if tables else "account_move_line"
                wheres = [""]
                if where_clause.strip():
                    wheres.append(where_clause.strip())
    ...
    def _compute_subgroup_balance(self, accounts,rep_sing):
        ...
        if rep_sing:
            ragrp = sorted(vagrp.items(), key = lambda kv:(kv[1], kv[0]),reverse=False)
        else:
            ragrp = sorted(vagrp.items(), key = lambda kv:(kv[1], kv[0]),reverse=True)

        for grupo,balance in ragrp:         
            sGrupo = '%04d' % (int(grupo) or 0)
            gdClave = "G%s"%sGrupo
            gtClave = "G%sT"%sGrupo            
            accxsubgrp = account_obj.search([('id','in',a_ids),('group_id','=',grupo)],order='subgroup_id')
            ...
            if accxsubgrp:
                ...
                if rep_sing:                    
                    rsagrp = sorted(sagrp.items(), key = lambda kv:(kv[1], kv[0]),reverse=False)
                else:
                    rsagrp = sorted(sagrp.items(), key = lambda kv:(kv[1], kv[0]),reverse=True)			
                if tiene_subgrupo:
        ...
        for account in accounts:
            ...
            for field in fields:
                ...
                if account_id.subgroup_id and account_id.group_id:                
                    ...
                    res[gtClave]['formula'] = 'result = %s'%(res[gtClave][field])
                    """                                                         
        return res     
        ...
        for report in reports:
            ...
            if report.formula:
                ...
                if report.subgroup_ids:
                    filtro.append(('subgroup_id','in',report.subgroup_ids.ids))                
                list_ids = self.env['account.account'].search(filtro)                
                res[report.id]['account'] = self._compute_account_balance(list_ids,filtro)
                for value in res[report.id]['account'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
                res[report.id]['subgroup'] = self._compute_subgroup_balance(res[report.id]['account'],report.reverse_order)                

            if report.sign and report.sign in ['+','-']:
                rep_sing = 1.00
                if report.sign in ['-']:
                    rep_sing = -1.00
                res[report.id]['balance'] = abs(res[report.id]['balance']) * rep_sing
    ...
    def get_account_lines(self, data):
        lines = []
        account_report = self.env['slm.financial.reports.define'].search([('id', '=', data['account_report_id'][0])])
        #child_reports = account_report._get_children_by_order()
        tag_id = data['tag_id'] and data['tag_id'][0] or False
        if tag_id:
            child_reports = account_report.report_line.filtered(lambda l: l.id == tag_id)
        else:
            child_reports = account_report.report_line
        res = self.with_context(data.get('used_context'))._compute_report_balance(child_reports)                       
        comparison_res = self.with_context(data.get('year_prior_context'))._compute_report_balance(child_reports)
        for report_id, value in comparison_res.items():
            res[report_id]['year_prior_bal'] = value['balance']
            if tag_id:
                res[report_id]['year_prior_subgroup'] = value['subgroup']
        comparison_res = self.with_context(data.get('month_prior_context'))._compute_report_balance(child_reports)
        for report_id, value in comparison_res.items():
        ...
        if tag_id:
            for report in child_reports:
                ...
                for subg_id in subg_rec:
                    ...
                    for x in cursor:
                        if x==1:
                            sg_name = subgroup_bal[subg_id]['name'] #record.name
                            sg_id = subg_id #record.id
                            sg_sing = ' ' #report.sign
                            sg_formula = subgroup_bal[subg_id]['formula']
                        else:
                            sg_name = _('It has no associated subgroup')
                            sg_id = 999999
                            sg_sing = ' '
                        ...
                        if (sg_id in subgroup_bal) or (sg_id in year_prior_subgroup) or (sg_id in month_prior_subgroup):
                            ...
                            if sg_id in subgroup_bal:
                                vals['balance'] = (subgroup_bal[sg_id]['balance'] if subgroup_bal[sg_id]['balance'] else 0.00)
                            if sg_id in year_prior_subgroup:
                                vals['year_prior_bal'] = (year_prior_subgroup[sg_id]['balance'] if year_prior_subgroup[sg_id]['balance'] else 0.00)
                            if sg_id in month_prior_subgroup:
                                vals['month_prior_bal'] = (month_prior_subgroup[sg_id]['balance'] if month_prior_subgroup[sg_id]['balance'] else 0.00)                            
            ...
            for report in child_reports:
                ...
                vals['percent_balance'] = (vals['balance']/incomes * 100 if incomes else 0.00)
                vals['percent_budget'] = (vals['budget']/incomes_budget * 100 if incomes_budget else 0.00)
                vals['percent_year_prior_bal'] = (vals['year_prior_bal']/incomes_year_prior_bal * 100 if incomes_year_prior_bal else 0.00)
                vals['percent_month_prior_bal'] = (vals['month_prior_bal']/incomes_month_prior_bal * 100 if incomes_month_prior_bal else 0.00)
                
                vals['percent_diff_budget'] = (vals['diff_budget']/vals['budget'] * 100 if vals['budget'] else 0.00)
                vals['percent_diff_year'] = (vals['diff_year']/vals['year_prior_bal'] * 100 if vals['year_prior_bal'] else 0.00)
                vals['percent_diff_month'] = (vals['diff_month']/vals['month_prior_bal'] * 100 if vals['month_prior_bal'] else 0.00)            
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        if company:
            logged_company = self.env['res.company'].browse(company[0]).partner_id
        sheet_report = [1]
        for x in sheet_report:
            if x==2:  
                res = report_res
            else:
                res = [1]
            for k in res:
                ...
                format22cb = workbook.add_format({'font_size': 14,'bg_color': solid_color})
                format22box_top = workbook.add_format({'font_size': 14,'bg_color': solid_color})
                format22box_mid = workbook.add_format({'font_size': 14,'bg_color': solid_color})
                format22box_bot = workbook.add_format({'font_size': 14,'bg_color': solid_color})
                ...
                sheet.merge_range('C6:D6', (data['form']['col_text_1']), format22box_top)
                sheet.merge_range('F6:G6', (data['form']['col_text_1']), format22box_top)
                sheet.merge_range('I6:J6', (data['form']['col_text_2']), format22box_top)
                sheet.merge_range('L6:M6', (data['form']['col_text_3']), format22box_top)                
                ...
                data_format1b = workbook.add_format({'bg_color': '#eaf4ff','font_size': 10,'bold': True})
                data_format2 = workbook.add_format({'bg_color': '#ffffff','font_size': 10})
                data_format2b = workbook.add_format({'bg_color': '#ffffff','font_size': 10,'bold': True})
                data_format1.set_bold(False)
                data_format2.set_bold(False)
                row_sum = 9 
                _logger.info('len(report_res):%s'%row_sum)
                row_sum = 1
                rec = 0
                for x in report_res:
                    rec += 1
                    if rec==len(report_res):
                        continue
                    if (x['type'] in ['detail','tag']) or (x['type']=='tittle' and not x['formula']):
                        row_sum += 1 
                    else: 
                        row_sum += 2
                row_sum += row_number
                ...
                for values in report_res:                      
                    if row_number%2 == 0:
                        ...
                        format3 = workbook.add_format({'bg_color': '#eaf4ff','font_size': 10})                
                        money = workbook.add_format({'bg_color': '#eaf4ff','font_size': 10,'num_format': '#,##0.00'})
                        percent_fmt = workbook.add_format({'bg_color': '#eaf4ff','font_size': 10,'num_format': '0.0'})
                    else:
                        ...
                        percent_fmt = workbook.add_format({'bg_color': '#ffffff','font_size': 10,'num_format': '0.0'})                  
                    if values['type']=='tittle':
                    ...
                    if values['type']=='tag' or (values['type']=='tittle' and not values['formula']):
                        for i in range(1,21):           
                            sheet.write(row_number, col_number+i, '', format3)                
                        row_number += 1
                        continue
                    sheet.write(row_number, col_number + 1, values['balance'], money)
                    sheet.write(row_number, col_number + 2, '=(C%s/C$%s)'%(str(row_number+1),str(row_sum)), percent_fmt) #values['percent_balance']
                    sheet.write(row_number, col_number + 4, values['budget'], money)
                    sheet.write(row_number, col_number + 5, '=F%s/F$%s'%(str(row_number+1),str(row_sum)), percent_fmt) #values['percent_budget']
                    sheet.write(row_number, col_number + 7, values['year_prior_bal'], money)
                    sheet.write(row_number, col_number + 8, '=I%s/I$%s'%(str(row_number+1),str(row_sum)), percent_fmt) #values['percent_year_prior_bal'] 
                    sheet.write(row_number, col_number + 10, values['month_prior_bal'], money)
                    sheet.write(row_number, col_number + 11, '=L%s/L$%s'%(str(row_number+1),str(row_sum)), percent_fmt) #values['percent_month_prior_bal']            
                    sheet.write(row_number, col_number + 13, values['diff_budget'], money)
                    sheet.write(row_number, col_number + 14, '=O%s/F%s'%(str(row_number+1),str(row_number+1)), percent_fmt) #values['percent_diff_budget']
                    sheet.write(row_number, col_number + 16, values['diff_year'], money)            
                    sheet.write(row_number, col_number + 17, '=R%s/I%s'%(str(row_number+1),str(row_number+1)), percent_fmt) #values['percent_diff_year']
                    sheet.write(row_number, col_number + 19, values['diff_month'], money)
                    sheet.write(row_number, col_number + 20, '=U%s/L%s'%(str(row_number+1),str(row_number+1)), percent_fmt) #values['percent_diff_month']
                    row_number += 1
                    if values['type']=='tittle':
                        if row_number%2 == 0:
                            sheet.set_row(row_number, cell_format=data_format1)
                ...
                if data['form'].get('tag_id', False):
                    ...
                    sheet.write(row_number,col_number + 1, '=SUM(C'+str(row_initial+2) +':C'+ str(row_sum) + ')-'+str(balance_dif), money)
                    sheet.write(row_number,col_number + 4, '=SUM(F'+str(row_initial+2) +':F'+ str(row_sum) + ')', money)
                    sheet.write(row_number,col_number + 7, '=SUM(I'+str(row_initial+2) +':I'+ str(row_sum) + ')-'+str(year_prior_bal_dif), money)
                    sheet.write(row_number,col_number + 10, '=SUM(L'+str(row_initial+2) +':L'+ str(row_sum) + ')-'+str(month_prior_bal_dif), money)
