...
class AccountLayoutProfitLossReport(models.TransientModel):
    _name = 'account.layoutprofitloss.report'
    _inherit = "account.common.report"
    _description = 'Report Layout Profit and Loss'
    ...
    def _get_account_report(self):
        ...
        if self._context.get('active_id'):
            ...
            reports = self.env['slm.financial.reports.define'].search([('name', 'ilike', menu)])
        return reports and reports[0] or False
     
    @api.model
    def _get_dates(self):          
        self.date_month_prior = self.date_to.replace(day=1) + relativedelta(days=-1)
    ...
    def _get_es_consolidate(self):
        self.es_consolidate = ('SLM CONSOLIDATED' == self.env.user.company_id.name)            
        
    enable_filter = fields.Boolean(string='Consolidated?',default=False)
    account_report_id = fields.Many2one('slm.financial.reports.define', string='Financial Reports', required=True, default=_get_account_report, domain=[('type','=','pyl')])
    ...
    date_month_prior = fields.Date(compute='_get_dates',string='End Date Month Prior')
    date_from_year_prior = fields.Date(compute='_get_dates',string='End Date Year Prior')
    date_to_year_prior = fields.Date(compute='_get_dates',string='Start Date Year Prior')
    ...
    tag_id = fields.Many2one('slm.financial.reports.define.lines','Tags Reports', domain=[('type','=','detail')])
    ...
    def default_get(self, fields):
        result = super(AccountLayoutProfitLossReport, self).default_get(fields)
        selected_companies = self.env['res.company'].search([])
        selected_report = self.env['slm.financial.reports.define'].search([('type','=','pyl')],limit=1)
        self.account_report_id_change()        
        result.update({
    ...
    def companies(self, id_company):
        selected_companies = self.env['res.company'].search([('parent_id','=',id_company)]).ids
    ...
    def company_id_change(self):
        ...
        if selected_companies:            
            xcia = self.env['res.company'].search([('id','=',selected_companies)]).ids
    ...
    def account_report_id_change(self):
        ...
        if self.account_report_id:
            report_tag_ids  =  self.env['slm.financial.reports.define.lines'].search([('report_id', '=', self.account_report_id.id)])
            filtro =  [x.id for x in report_tag_ids]
            #domain = {'tag_id': [('id', 'in', filtro),('type','=','detail')]}                          
            domain = {'tag_id': [('report_id', '=', self.account_report_id.id),('type','=','detail')]}
    ...
    def _build_month_prior_context(self, data):
        ...
        #dto = data['form']['date_month_prior'] + relativedelta(months=-1)
        #result['date_from'] = dto + relativedelta(months=-1) + relativedelta(days=1)
        #result['date_to'] = self.date_month_prior          
        result['strict_range'] = True
        return result
    ...
    def check_report(self):
        ...
        res = super(AccountLayoutProfitLossReport, self).check_report()
        data = {}
        data['form'] = self.read(['date_month_prior','date_from_year_prior','date_to_year_prior','account_report_id', 'date_from_cmp', 'date_to_cmp', 'journal_ids', 'filter_cmp', 'target_move','enable_consolidate','multi_company_ids'])[0]
        ...
        used_context = self._rebuild_used_context(data,res['data']['form']['used_context'])
    ...
    def _print_report(self, data):
        context = self._context
        data['form'].update(self.read(['tag_id','col_text_1','col_text_2','col_text_3','date_from_cmp', 'debit_credit', 'date_to_cmp', 'filter_cmp', 'account_report_id', 'enable_filter', 'label_filter', 'target_move','enable_consolidate','multi_company_ids'])[0])
        if context.get('xls_export'):            
            return self.env.ref('slm_report_lpl.action_layoutprofitloss_xls').report_action(self, data=data, config=False)
        return self.env.ref('slm_report_lpl.action_layoutprofitloss').report_action(self, data=data, config=False)
