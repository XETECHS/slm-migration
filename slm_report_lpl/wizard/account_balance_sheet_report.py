...
class AccountLayoutBalanceSheetReport(models.TransientModel):
    ...
    account_report_id = fields.Many2one('slm.financial.reports.define', string='Financial Reports', required=True, default=_get_account_report, domain=[('type','=','bls')])
    ...
    def default_get(self, fields):
        result = super(AccountLayoutBalanceSheetReport, self).default_get(fields)
        selected_companies = self.env['res.company'].search([])
        selected_report = self.env['slm.financial.reports.define'].search([('type','=','bls')],limit=1)        
        result.update({
