...
class AccountBankStatement(models.Model):
    ...
    def _get_opening_balance(self, journal_id):
        curr_user_id = self.env['res.users'].browse(
            self.env.context.get('uid', False))
        last_bnk_stmt = self.search(
            [('journal_id', '=', journal_id), ('branch_id', '=', curr_user_id.branch_id.id)], limit=1)
        if last_bnk_stmt:
    ...
    def default_get(self, fields):
        ...
        res['branch_id'] = self.env['res.users'].browse(
            self.env.uid).branch_id.id
        return res
        ...
        for statement in statements:
            ...
            if moves:
                if self._context.get('session'):
                    ...
                    statement.write(
                        {'branch_id': statement.pos_session_id.branch_id.id})
                else:
                    moves.filtered(lambda m: m.state != 'posted').post()
                    for move in moves:
            ...
            statement.message_post(body=_(
                'Statement %s confirmed, journal items were created.') % (statement.name,))
