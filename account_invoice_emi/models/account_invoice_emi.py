...
class Account_Invoice_EMI(models.Model):
    ...
    so_id = fields.Many2one('sale.order', string="Sales Order", required=False, domain="[('state', '=', 'done')]", track_visibility='onchange')
    type = fields.Selection([('fixed', 'Fixed'), ('manual', 'Manual')], string="EMI Type", default="manual", track_visibility='onchange')
    total = fields.Integer(string="Total EMI", track_visibility='onchange')
    total_emi = fields.Integer(string="Total EMI", readonly=True, track_visibility='onchange')
    ...
    journal_id = fields.Many2one('account.journal', string="Payment Journal", track_visibility='onchange')
