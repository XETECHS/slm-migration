    ...
    def _post_process(self, grouped_accounts, initial_balances, options, comparison_table):
        ...
        for account in sorted_accounts:
            ...
            if int(account.code[0]) >= 4 and account.code != '999999':
                cols += [{'name': zero_value, 'no_format_name': zero_value}, {'name': zero_value, 'no_format_name': zero_value}]
        ...
        difference_columns[-2] = {'name': self.format_value(sum_columns[-2] - sum_columns[-1])}
        difference_columns[-4]  = {'name': self.format_value(sum_columns[-4] - sum_columns[-3])}
        ...
        for period in reversed(comparison_table):
            ...
            for account in res:
                ...
                if account.code not in account_codes:
                    ...
                    grouped_accounts[parent_account][period_number]['balance'] += res[account]['balance'] - res[account]['initial_bal']['balance']
                    grouped_accounts[parent_account][period_number]['debit'] += res[account]['debit'] - res[account]['initial_bal']['debit']
                    grouped_accounts[parent_account][period_number]['credit'] += res[account]['credit'] - res[account]['initial_bal']['credit']
